...
import {IProxyCreationCallback} from "safe-contracts/contracts/proxies/IProxyCreationCallback.sol";
 ...
 */
contract SafeProxyFactoryBrokenDeployment {
     ...
     * @param saltNonce Nonce that will be used to generate the salt to calculate the address of the new proxy contract.
     */
    function createProxyWithNonceExtraCREATE2Opcode(address _singleton, bytes memory initializer, uint256 saltNonce) public returns (SafeProxy proxy) {
        // If the initializer changes the proxy address should change too. Hashing the initializer data is cheaper than just concatinating it
        bytes32 salt = keccak256(abi.encodePacked(keccak256(initializer), saltNonce));
        proxy = deployProxy(_singleton, initializer, salt);

        // This line breaks ERC 4337 rules as multiple CREATE2 opcodes are required in initCode
        TestContract testContract = new TestContract{salt: keccak256(abi.encode("I'm a salt"))}();
        emit ProxyCreation(proxy, _singleton);
    }
     ...
     * @param saltNonce Nonce that will be used to generate the salt to calculate the address of the new proxy contract.
     */
    function createProxyWithNonceWithCREATEOpcode(address _singleton, bytes memory initializer, uint256 saltNonce) public returns (SafeProxy proxy) {
        // If the initializer changes the proxy address should change too. Hashing the initializer data is cheaper than just concatinating it
        bytes32 salt = keccak256(abi.encodePacked(keccak256(initializer), saltNonce));
        proxy = deployProxy(_singleton, initializer, salt);

        // This line breaks ERC 4337 rules as the banned 'CREATE' opcode is used
